...
contract WrappedToken is Context, IERC20 {
    ...
    ///////////////
    //
    // ERC20 Custom external functions (privileged to initial deployer address)
    //
    ///////////////

    /**
     * @dev Mints `amount` of tokens to `sender` on deposit.
     */
    function mint(address receiver, uint amount) external override {
        require(receiver != address(0), "ERC20: mint to zero addr");
        require(msg.sender == _deployer, "Don't have rights");
        _mint(receiver, amount);
    }
    
    /**
     * @dev Burns `amount` of tokens from `sender` on withdraw.
     */
    function burn(address burner, uint amount) external override {
        require(burner != address(0), "ERC20: burn from zero addr");
        require(msg.sender == _deployer, "Don't have rights");
        _burn(burner, amount);
    }
